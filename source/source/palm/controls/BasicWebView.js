	...
	*/
	...
	callBrowserAdapter: function(inFuncName, inArgs) {
		if (this.hasNode() && this.node[inFuncName] && this._serverConnected) {
			// make sure the call queue is flushed
			this.flushCallQueue();
	...
	flushCallQueue: function() {
		...
		if (this.callQueue.length > 0) {
			// callBrowserAdapter() below may call back into flushCallQueue, so we take ownership of the whole call queue here
			// in order to prevent nested calls to flushCallQueue from emptying the array while we're still trying to iterate over it.
			var callQueue = this.callQueue;
			this.callQueue = [];
			if (this.hasView()) {
				var l = callQueue.length;
				for (var i=0, q; i<l; i++) {
					q = callQueue.shift();
